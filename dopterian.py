...
def ferengi_convolve_plus_noise(image,psf,sky,exptime,nonoise=False,border_clip=None,extend=False):
    ...
    if nonoise==False:
        ef= Nout%2
        try:
            out+=sky[Nsky//2-Nout//2:Nsky//2+Nout//2+ef,\
                     Msky//2-Mout//2:Msky//2+Mout//2+ef]+\
...
def ferengi(imgname,background,lowz_info,highz_info,namesout,imerr=None,noflux=False,evo=None,noconv=False,kcorrect=False,extend=False,nonoise=False,write_results=False,border_clip=3):
    ...
    if kcorrect:
        ...
        print('IM_NOK',np.sum(img_nok),np.amax(img_nok),np.amin(img_nok))
        ...
        print('IM_DOWNSCALE',np.sum(img_downscale),np.amax(img_downscale),np.amin(img_downscale))
    ...
    print('IM_DOWNSCALE_median_nans',np.sum(img_downscale),np.amax(img_downscale),np.amin(img_downscale))

    idx = np.where(img_downscale==0.0)
    img_downscale[idx]=median[idx]
    print('IM_DOWNSCALE_median_zeros',np.sum(img_downscale),np.amax(img_downscale),np.amin(img_downscale))
    ...
    print('IM_DOWNSCALE_sky',np.sum(img_downscale),np.amax(img_downscale),np.amin(img_downscale))
    ...
    print('PSF_transformation',np.sum(psf_t),np.amax(psf_t),np.amin(psf_t))
    ...
    try:
        ...
        return -99,-99
##    pyfits.writeto('transform_psf_dopterian.fits',psf_t,overwrite=True)
    print('PSF_reconstruction',np.sum(recon_psf),np.amax(recon_psf),np.amin(recon_psf))
    ...
    print('Final Image',np.sum(img_downscale),np.amax(img_downscale),np.amin(img_downscale))
